<template>
  <div>
    <v-list-item v-if="!isPublic" v-bind:class="{ read: note.readAt }" @click="readNote(note)">
      <span style="color: #00BFFE">{{ note.nickname }}</span
      >님이 <span style="color: #CE1D28"> 팔로우</span> 요청을 보냈습니다.
      <v-btn
        v-if="!accepted"
        small
        class="ms-2"
        style="background-color:#00BFFE; color:white"
        @click="acceptFollow"
        >수락</v-btn
      >
    </v-list-item>
    <v-list-item v-else v-bind:class="{ read: note.readAt }" @click="readNote(note)">
      <span style="color: #00BFFE">{{ note.nickname }} </span>님이
      <span style="color: #CE1D28"> 팔로우</span>하셨습니다.
    </v-list-item>
  </div>
</template>
        ...
        .then((res) => {
          ...
          axios({
            ...
            .then((res) => {
              ...
              let idx = this.myAssets.findIndex(function(ob) {
                return ob.notificationIdx === notification_idx;
              });
