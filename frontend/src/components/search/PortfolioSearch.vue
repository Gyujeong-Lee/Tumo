<template>
  <div v-if="portfolios.length">
    ...
    <div class="d-flex flex-column flex-sm-row my-auto">
      <v-card :elevation="elevation" shaped v-for="(portfolio, idx) in portfolios" :key="idx" class="me-3 mt-3">
        ...
        <v-card-text>총 자산: {{ (portfolio.cursum).toLocaleString() }}</v-card-text>
        <v-card-text>수익률: {{ (portfolio.percent).toFixed(2) }}%</v-card-text>
      </v-card>
    </div>
  </div>
  <div v-else>
...
export default {
  ...
  created: function () {
    ...
    axios({
      method: 'GET',
      url: `/api/portfolio/search/${this.$route.params.keyword}`
    })
    .then(res => {
      // console.log(res)
