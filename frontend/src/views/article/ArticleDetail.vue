<template>
  <div v-if="!isLoading" class="d-flex justify-content-center">
    <v-sheet elevation="8" rounded id="articleDetail">
          ...
          <div class="ms-3">
            ...
            <div style="font-size: 0.9rem;">
              ...
              <span class="mx-3 text-primary"
                ><v-icon small color="primary">mdi-chart-bar</v-icon> {{ $data.stock }}</span
              >
            </div>
          </div>
        </div>
      ...
      <div class="mb-3">
        <v-chip v-for="(tag, idx) in $data.tags" :key="idx" label class="px-2 me-2 mb-2"
          >#{{ tag }}</v-chip
        >
      </div>
      <!-- Btn Group -->
      <div class="d-flex justify-content-between">
        <div>
          <v-btn icon v-if="$data.isLike" @click="cancelLikeArticle" color="error"
            ><v-icon color="error">mdi-heart</v-icon></v-btn
          >
          <v-btn icon v-else @click="likeArticle"><v-icon>mdi-heart-outline</v-icon></v-btn>
          <span style="font-size: 0.85rem;">{{ $data.likes }}</span>
        </div>
        <div>
          <v-btn icon v-if="$data.isScrap" @click="cancelScrapArticle" color="yellow"
            ><v-icon color="yellow">mdi-bookmark</v-icon></v-btn
          >
        ...
        <div>
          ...
          >
            <v-btn icon slot="reference" @click="drawDeleteArticle"
  ...
  methods: {
    ...
    deleteArticle: function() {
      axios({
        ...
        this.$alert("게시물이 성공적으로 삭제되었습니다.", "삭제 성공", "success").then(() => {
