...
public class CompanyServiceImpl implements CompanyService {
	...
	public List<Map<String, Object>> corpReport(String corp_code) {
		...
		try {
			URL url = new URL("https://opendart.fss.or.kr/api/fnlttSinglAcnt.json?crtfc_key=" + key + "&corp_code="
					+ corp_code + "&bsns_year=" + (getLocalDate.getYear() - 1) + "&reprt_code=11011");
			BufferedReader bf;
			bf = new BufferedReader(new InputStreamReader(url.openStream(), "UTF-8"));
			result = bf.readLine();
		} catch (Exception e) {
...
//		System.out.println("https://opendart.fss.or.kr/api/fnlttSinglAcnt.json?crtfc_key=" + key + "&corp_code="
//				+ corp_code + "&bsns_year=" + (getLocalDate.getYear() - 1) + "&reprt_code=11011");
//		System.out.println(result);
			...
			for (int i = 0; i < item.size(); i++) {
				...
				if (tmp.get("fs_div").toString().equals("OFS")) {
					...
					if (tmp.get("account_nm").toString().equals("당기순이익")) {
						tmpmap = putmap(tmp, tmpmap, "당기순이익");
						list.add(tmpmap);
						roe[0] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						roe[1] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						roe[2] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}
					if (tmp.get("account_nm").toString().equals("유동자산")) {
						moveList[0] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						moveList[1] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						moveList[2] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}
					if (tmp.get("account_nm").toString().equals("유동부채")) {
						moveList[3] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						moveList[4] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						moveList[5] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}

					if (tmp.get("account_nm").toString().equals("부채총계")) {
						debt[0] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						debt[1] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						debt[2] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}
					if (tmp.get("account_nm").toString().equals("자본총계")) {
						debt[3] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						debt[4] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						debt[5] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}
					if (tmp.get("account_nm").toString().equals("매출액")) {
						roe[3] = Long.parseLong(
								tmp.get("thstrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						roe[4] = Long.parseLong(
								tmp.get("frmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
						roe[5] = Long.parseLong(
								tmp.get("bfefrmtrm_amount").toString().replace(",", "").replace("-", "-0").trim());
					}
